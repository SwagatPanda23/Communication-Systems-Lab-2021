...
if rand_num <= 3
    ...
    for T = 0:(floor(length(data)/fs))  % Duration 30 seconds with interval of 1 sec.
        if (T == 0)
            disp('Transmission Started')
            disp(T)
        elseif(T == duration_signal) 
        ...
        ylabel('Magnitude')
        grid on
        % First two are limits for x-axis, the other two are limits for y-axis: observe why -inf inf , and 0 3 are used here. 
